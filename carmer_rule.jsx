...
function Carmer(){
    ...
    return(
        <div className="flex flex-col items-center justify-center min-h-screen space-y-4 p-4">
            <h1 className="font-bold text-3xl">Carmer's Rule</h1>
            <div className="flex items-center gap-3 p-5 rounded-2xl shadow-md">
                <label className="block text-lg mb-1">Matrix size (NxN)</label>
                <input type="number"
                className="shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400 p-2 w-20 text-center rounded-lg"
                min={1}
                value={n}
                onChange={(e) => Changesize(e.target.value)} />
                <button onClick={Calculatecarmer} className="bg-blue-500 hover:bg-blue-600 px-6 py-3 rounded text-white sm:w-auto">Calculate</button>
            </div>
