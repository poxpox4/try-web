...
function Jacobi(){
    ...
    function Calculate(){
        try{
            ...
            do{
                ...
                for(let i=0;i<n;i++){
                    errorcal[i] = Math.abs((xk1[i]-xold[i])/xk1[i]);
                }
                for(let i=0;i<n;i++){
                    xk0[i] = xk1[i];
                }
    ...
    return(
        <div className="flex flex-col items-center justify-center space-y-4 min-h-screen p-4">
            <h1 className="font-bold text-3xl">Jacobi</h1>
            <div className="flex items-center gap-3 p-5 rounded-2xl shadow-md">
                <label className="">Matrix size(NxN)</label>
                <input type="number" 
                min={1}
                placeholder="1"
                className="rounded-lg shadow-md p-4 w-20 text-center"
                value={n}
                onChange={(e) => Changesize(e.target.value)}/>
                <label >Error</label>
                <input type="number" 
                placeholder="0.00001"
                className="shadow-md p-4 w-20 text-center rounded-lg"
                value={error}
                onChange={(e) => setError(e.target.value)}/>
                <button onClick={Calculate} className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded sm:w-auto">Calculate</button>
                
            </div>
            <h2 className="block">X start</h2>
            <div className="flex items-center">
                ...
                {xt.map((value,i)=>(
                    <div className="flex gap-2 mb-1">
                        <input type="text" 
                        placeholder={`x${i+1}`}
                        className="shadow-md p-7 w-20 text-center mb-1 rounded-lg"
                        value={value}
                        onChange={(e) => ChangeXt(i,e.target.value)}/>
                    </div>
                ))}
            </div>
            <div className="flex md:flex-row gap-6 w-full items-center justify-center">
                ...
                <div className="flex flex-col items-center">
                    ...
                    {Array.from({length:n},(_,i)=>(
                        <div key={i} className="flex gap-2 mb-1">
                            <div className="shadow-md p-7 w-20 text-center mb-1 rounded">
                                x{i+1}
                            </div>
                        </div>
                    ))}
                ...
                <div className="flex flex-col items-center">
                    ...
                    {matrixB.map((value,i)=>(
                        <div className="flex gap-2 mb-1">
                            ...
                            key={i}
                            placeholder={`b${i+1}`} 
                            className="shadow-md p-7 w-20 text-center mb-1 rounded"
                            value={value}
